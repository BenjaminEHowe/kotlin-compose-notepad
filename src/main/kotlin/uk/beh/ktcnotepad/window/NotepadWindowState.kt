...
import uk.beh.ktcnotepad.NotepadApplicationState
import uk.beh.ktcnotepad.common.Settings
import uk.beh.ktcnotepad.util.AlertDialogResult
...
private fun Path.launchSaving(text: String) =
    GlobalScope.launch {
        writeTextAsync(text)
    }

private suspend fun Path.writeTextAsync(text: String) =
    withContext(Dispatchers.IO) {
        toFile().writeText(text)
    }

private suspend fun Path.readTextAsync() =
    withContext(Dispatchers.IO) {
        toFile().readText()
    }
